#!/usr/bin/env python3
"""
STEP 2: Load charges npzs (STEP 1) and fit pedestal + SPE gaussians.
...
# ----------------- LIST PMTs -----------------
charge_files = [f for f in os.listdir(in_dir) if fnmatch.fnmatch(f, args.pattern)]
pmts_all = sorted([f.replace(".npz","") for f in charge_files])
...
for idx, pmt_label in enumerate(pmts_all[start_idx:end_idx], start=start_idx):
    try:
        ...
        charges_npz = os.path.join(in_dir, pmt_label + ".npz")
        data = np.load(charges_npz)
        charges = data["charges"]
        pulse_ratio = data["pulse_ratio"]
        mu_pe = data["mu_pe"]
...
np.savez(os.path.join(out_dir, f"Final_run2306_770ps_chunk{chunk_id}.npz"), results=results_array)

failed_file = os.path.join(out_dir, f"failed_pmts_run2306_770_chunk{chunk_id}.json")
